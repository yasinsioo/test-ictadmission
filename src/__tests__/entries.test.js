...
// Use a unique temp file per test run — avoids :memory: isolation issues
...
beforeAll(() => {
  // Schema already created by db.js; just seed one row
  db.prepare(
    'INSERT INTO entries (title, body, lat, lon, isoTime) VALUES (?, ?, ?, ?, ?)'
  ).run('Night perimeter check', 'All clear.', 60.15, 25.02, new Date().toISOString());
});
...
describe('POST /api/entries', () => {
  ...
  it('returns 400 when title is missing', async () => {
    const res = await request(app).post('/api/entries').send({ body: 'No title here' });
    expect(res.status).toBe(400);
    expect(res.body.error).toBe('title is required');
  });

  it('returns 400 when body is missing', async () => {
    const res = await request(app).post('/api/entries').send({ title: 'No body' });
    expect(res.status).toBe(400);
